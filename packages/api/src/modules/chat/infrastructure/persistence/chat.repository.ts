...
import {
  ...
  PaginatedSessions,
  PaginatedMessages,
} from '../../domain/repositories/chat.repository.port';
import { ChatSessionEntity, ChatMessageEntity } from '../../domain/entities/chat.entities';
...
export class ChatRepository implements ChatRepositoryPort {
  ...
  async findSessionById(id: string): Promise<ChatSessionEntity | null> {
    ...
    return session ? new ChatSessionEntity({
      ...session,
      paperId: session.paperId ?? undefined,
    }) : null;
  }
