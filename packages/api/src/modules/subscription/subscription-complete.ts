 ...
 */
import { Module, Injectable, Controller, Post, Delete, Patch, Get, Param, Body, UseGuards, Req } from '@nestjs/common';
...
export class SubscribeDto {
  @ApiProperty({ enum: ['CATEGORY', 'TAG', 'JOURNAL', 'AUTHOR'] })
  @IsEnum(['CATEGORY', 'TAG', 'JOURNAL', 'AUTHOR'])
  type: string;
...
export class SubscriptionController {
  ...
  async subscribe(@Body() dto: SubscribeDto, @Req() req: Request & { user: any }) {
    return await this.subscriptionFacade.subscribe(req.user.id, dto.type, dto.target);
  }

  @Delete(':subscriptionId')
  @ApiOperation({ summary: 'Unsubscribe' })
  async unsubscribe(@Param('subscriptionId') subscriptionId: string, @Req() req: Request & { user: any }) {
  ...
  async toggleSubscription(@Param('subscriptionId') subscriptionId: string, @Req() req: Request & { user: any }) {
  ...
  async listSubscriptions(@Req() req: Request & { user: any }) {
    return await this.subscriptionFacade.listSubscriptions(req.user.id);
  }
}

// Module
const commandHandlers = [SubscribeHandler, UnsubscribeHandler, ToggleSubscriptionHandler];
const queryHandlers = [ListSubscriptionsHandler];

@Module({
  imports: [CqrsModule, PrismaModule, UserModule],
  providers: [...commandHandlers, ...queryHandlers, SubscriptionFacade],
